...
//    @Pointcut("execution(* com.cykj.service.impl.*Impl.*(..))")
   //  @Pointcut("execution(* com.example.nba.repository.PlayerRep.*(..))")
    //切入点签名的方法，注意返回值必须是void,相当于切入点的无参构造
    @Pointcut("@annotation(com.cykj.service.log.OperateLog)")
...
//        String name = ((MethodSignature)jp.getSignature()).getMethod().getAnnotation(OperateLog.class).methodName();
//        String type = ((MethodSignature)jp.getSignature()).getMethod().getAnnotation(OperateLog.class).operationType().toString();
//
//        logger.info("methodName="+name+"/type="+type);
...
//        TabAdmin tabAdmin = (TabAdmin)result;
//        logger.info(tabAdmin.getAdminName());
//        TabLog tabLog = new TabLog();
//        tabLog.setOperator(tabAdmin.getAdminName());
//        tabLog.setOperationItem("登录");
//        logMapper.inertLog(tabLog);
//        sqlSession.commit();
//        sqlSession.close();
        ServletRequestAttributes attributes = (ServletRequestAttributes) RequestContextHolder.getRequestAttributes();
        HttpServletRequest request = attributes.getRequest();
